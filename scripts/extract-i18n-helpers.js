// Helper script to scan frontend source for hardcoded JSX text nodes
// Produces a JSON mapping of candidate keys -> strings for translation.
// Usage: node scripts/extract-i18n-helpers.js [--apply]
...
function extractStrings(content) {
  // crude regex to match >Some text< between tags (ignores braces)
  ...
  while ((m = regex.exec(content))) {
    const txt = m[1].trim();
    if (!txt) continue;
    // filter out likely code fragments or single chars
    if (txt.length < 2) continue;
    // skip if contains { or } (JS expression)
