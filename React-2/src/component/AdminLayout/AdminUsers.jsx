...
const AdminUsers = () => {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      if (statusFilter) {
        params.isActive = statusFilter === "active" ? true : statusFilter === "suspended" ? false : undefined;
      }
  ...
  const handleRoleChange = async (userId, newRole) => {
    try {
      ...
      setUsers((prev) =>
        prev.map((user) => (user.id === userId ? { ...user, role: normalizeRole(newRole) } : user)),
      );
      showToast({
        title: "Role updated",
          ...
          <>
            {users.length === 0 ? (
              <div className="rounded-[22px] border border-dashed border-white/10 bg-slate-900/70 p-10 text-center">
                ...
                <p className="mt-3 text-sm text-slate-400">Try adjusting the current filters.</p>
                <button
                  ...
                  <table className="min-w-[980px] w-full table-fixed text-left">
                    ...
                    <thead className="bg-slate-950/70">
                      ...
                      <tr className="border-t border-white/10 bg-slate-950/45 align-top">
                        ...
                        <th className="px-4 py-3">
                          <div className="px-1 py-2 text-xs text-slate-500">Matches name or email</div>
                        </th>
                        <th className="px-4 py-3">
                          <select
                            value={roleFilter}
                        ...
                        <th className="px-4 py-3">
                          <div className="px-1 py-2 text-xs text-slate-500">Assign role directly in-row</div>
                        </th>
                    ...
                    <tbody className="divide-y divide-white/5 text-sm text-slate-200">
                      {users.map((user) => {
                        ...
                        const statusKey = typeof user.isActive === "undefined" ? "" : user.isActive ? "active" : "suspended";
                          ...
                          <tr key={user.id} className="transition hover:bg-white/[0.03]">
                            ...
                            <td className="px-4 py-4 align-top">
                              <span
                                className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1 ${
                                  roleBadgeClassNames[normalizedRole] || "bg-white/10 text-slate-200 ring-white/10"
                                }`}
                              >
                                {normalizedRole}
                              </span>
                            ...
                            <td className="px-4 py-4 align-top">
                              {statusKey ? (
                                ...
                                >
                                  {statusKey === "active" ? "Active" : "Suspended"}
                                </span>
                              ) : (
