  ...
  useEffect(() => {
    const bootstrapContactsFromBackend = async () => {
      try {
        ...
        const contactsRes = await fetch(apiUrl(`/api/social/contacts/${latestRecord._id}`));
  ...
  const addContact = async () => {
    ...
    if (socialDataId) {
      try {
        ...
        setContacts(serverContacts.map((contact) => ({
          name: contact.name,
          number: String(contact.number)
        })));
      } catch {
        setContacts([...contacts, { name: newContactName.trim(), number: normalizedNumber }]);
      }
    } else {
      setContacts([...contacts, { name: newContactName.trim(), number: normalizedNumber }]);
    }
