...
const VerifyPhonePage = () => {
  ...
  const [step, setStep] = useState("verify"); // verify or enter-email
  ...
  const handleVerifySubmit = async (e) => {
    ...
    try {
      if (purpose === "login") {
        await verifyLoginOTPFunc({ contact: inputValue, otp, method: "sms" });
        navigate("/");
      } else {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/verify-email?contact=" + email);
    } catch (err) {
      ...
      >
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-semibold">
            {step === "verify" ? "Verify Phone" : "Enter Email"}
          </h2>
